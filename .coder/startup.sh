...
# Clone infra repo
if [ ! -d "shtetl-infra/.git" ]; then
    print_status "Cloning shtetl-infra..."
    rm -rf shtetl-infra 2>/dev/null  # Remove empty directory if exists
    git clone ${SHTETL_INFRA_REPO} shtetl-infra || print_warning "Failed to clone shtetl-infra"
else
...
if [ -n "${SHTETL_BRANCH}" ] && [ "${SHTETL_BRANCH}" != "main" ]; then
    ...
    for repo in shtetl shtetl-infra shtetl/submodules/shtetl-api shtetl/submodules/shtetl-web shtetl/submodules/shtetl-mobile; do
        if [ -d "$repo" ]; then
            cd "$repo"
...
echo "  - Zmanim: REST ${ZMANIM_REST_PORT}, gRPC ${ZMANIM_GRPC_PORT}"
echo "  - Shul: REST ${SHUL_REST_PORT}, gRPC ${SHUL_GRPC_PORT}"
